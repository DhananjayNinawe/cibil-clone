...
/**
 * Shared Knowledge Center blog-listing layout: hero band + gradient image, "Topics" grid of blog cards,
...
export default function BlogGrid({ titleKey, gradient = "from-[#8fd0ea] to-[#3a7a9a]", ctaKey = "blogPostLink", cards }: BlogGridProps) {
  const { t } = useLanguage();
    ...
    <>
      <section className="grid grid-cols-1 lg:grid-cols-2 bg-gray-100">
        <div className="flex flex-col justify-center px-4 sm:px-8 lg:px-16 py-16">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900">{t(titleKey)}</h1>
        </div>
        <div className={`relative w-full h-full min-h-[220px] overflow-hidden bg-gradient-to-br ${gradient}`} />
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10">
          {cards.map((title, i) => (
            <article key={`${title}-${i}`} className="flex flex-col">
              <div className="w-full aspect-[16/7] rounded bg-gradient-to-br from-[#cdeffb] to-[#8fd0ea] mb-3" />
