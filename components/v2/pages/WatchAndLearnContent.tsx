"use client";

import { WATCH_LEARN_CARDS } from "@/lib/blogCards";
import { useV2 } from "@/lib/v2/useV2";
import { toV2 } from "@/lib/v2/routes";
import { Container, Section, SectionHeading } from "@/components/v2/ui/Layout";
import PageHero from "@/components/v2/ui/PageHero";
import Button from "@/components/v2/ui/Button";
import HeroBand from "@/components/v2/pages/knowledge/HeroBand";
import KnowledgeGrid from "@/components/v2/pages/knowledge/KnowledgeGrid";
import KnowledgeOutro from "@/components/v2/pages/knowledge/KnowledgeOutro";
import TopicRail from "@/components/v2/pages/knowledge/TopicRail";

/** V1's hero banner for this listing. */
const HERO_IMAGE =
  "https://www.cibil.com/content/dam/cibil/consumer/Blog-Banner-1440x460_Watchlearn.jpg";

/**
 * Watch and Learn — the screening room. Centered title on the deepest tone, a cinemascope band
 * under it, and four oversized tiles rather than a dense grid: this is the page you browse, not
 * the one you search.
 */
export default function WatchAndLearnContent() {
  const { t } = useV2();

  return (
    <>
      <PageHero
        size="md"
        tone="deep"
        align="center"
        eyebrow={t("navKnowledge")}
        title={t("watchLearnTitle")}
        lede={t("learnSubtitle")}
        breadcrumbs={[{ label: t("navKnowledge") }, { label: t("watchLearnTitle") }]}
        actions={
          <Button href={toV2("/jaagran")} variant="ghost" arrow>
            {t("jaagranWatchMore")}
          </Button>
        }
      />

      <HeroBand src={HERO_IMAGE} />

      <TopicRail />

      <Section space="lg" tone="canvas">
        <Container>
          <SectionHeading index="01" eyebrow={t("formatVideo")} title={t("topicsHeading")} />
          <KnowledgeGrid
            className="mt-14"
            cards={WATCH_LEARN_CARDS}
            ctaKey="blogPostLink"
            href={toV2("/blog/main")}
          />
        </Container>
      </Section>

      <KnowledgeOutro />
    </>
  );
}
